...
Using `ggplot2` we can create a histogram of the `pdxTrees` inventory dates. All the trees seem to be inventoried in the summer months when the weather is nice in Portland. 
...
mod <- lm(pollution_removal_value ~ tree_height, data = pdxTrees)
...
Looking at the coefficients from the regression table it does seem like tree height has an affect on pollution removal. An intercept term of $\hat\beta_0 = 1.078$, implies when the tree_height equals zero the `pollution_removal_value` equals 1.078. Obviosuly you wouldn't have a tree that is zero feet tall, but this represents the y-intercept if we graph the regression. $\hat\beta_1 = 0.163$ implies that when the `tree_height` increases by one ft. the `pollution_removal_value` increases by 0.163. The $ \hat\beta_1 $ coefficent represents our slope. The null hypothesis is tree height has no affect on pollution removal and then alternative hypothesis is tree height has an affect on pollution removal. With a p-value of zero we can reject the null hypothesis at a 1\% significance level and support the alternative hypothesis that tree height does have an affect on pollution removal. We can visualize this relationship by plotting the data below with `ggplot2`. 

```{R linear regression graph, warning = FALSE}

# visualizing the relationship between the two variables. 

ggplot(pdxTrees, aes(x = tree_height, y = pollution_removal_value )) + 
...
I am 95% confident that the correlation coefficient lies between 0.611 and 0.63 This means that `tree_height` and `pollution_removal_value` are moderatly postivly correlated. An increase in `tree_height` results in an increase of `pollution_removal_value`, but it is not a strict one to one relationship. Visually we can see that our bootstrap distribution is bellshaped, symmmetric, and centered around the correlation coefficient. Even if our coefficent is the lower bound of our confidence interval there is still a moderate positive correlation. 
...
Unsurprisingly it seems that large have the highest `pollution_removal_value`, but it does seem like there is a lot of overlap between the height categories. 
